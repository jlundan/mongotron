...
export class ConnectionRepository {
    public static initialize(config: Config): Promise<Connection[]> {
        const configPath = path.join(config.userHomeDirectory, `${config.applicationName}_${config.schemaVersion}`, CONFIG_FILE_NAME);
        return FileUtils.readJson(configPath).then((connections: Connection[]) => {
    ...
    public static writeConnections(){
        console.log("writing");
    }
