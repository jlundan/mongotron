        ...
        return new Promise((resolve, reject) => {
            ConnectionValidator.validateCreate(newConnection)
                .then(() => {
                    ...
                    return this.connectionRepository.existsByName(newConnection.name);
                })
                .then((exists) => {
                    return new Promise((resolve, reject) => {
                ...
                .then(() => {
                    return this.connectionRepository.create(newConnection);
                })
                .then(resolve)
                .catch(reject);
        });
    ...
    update(id, updates) {
        ...
        return new Promise((resolve, reject) => {
            ...
            _this.findById(id)
                ...
                .then((connection) => {
                    this.connectionRepository.update(id, connection)
                        .then(resolve)
                        .catch(reject);
                })
                .catch(reject);
...
function _applyConnectionUpdatesPreValidation(connection, updates) {
    return new Promise((resolve) => {
        ...
        if (!MongoUtils.isLocalHost(connection.host)) {
            ...
            if (!db) LoggerService.warn('connection service - _applyConnectionUpdates() - connection has no database');
            else {
